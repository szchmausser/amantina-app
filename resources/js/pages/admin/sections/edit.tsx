...
export default function SectionEdit({ section, grades, academicYears }: Props) {
    ...
    const { data, setData, post, put, processing, errors } = useForm({
        academic_year_id: section?.academic_year_id || (defaultYearId ? parseInt(defaultYearId) : (grades[0]?.academic_year_id || academicYears[0]?.id)),
        grade_id: section?.grade_id || (defaultGradeId ? parseInt(defaultGradeId) : grades[0]?.id),
        name: section?.name || '',
    });
    ...
    const filteredGrades = grades.filter(g => g.academic_year_id === data.academic_year_id);

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={isEditing ? `Editar Sección: ${section.name}` : 'Nueva Sección'} />
            ...
            <div className="mx-auto max-w-2xl p-4 lg:p-8">
                <div className="mb-6">
                    ...
                    <p className="text-sm text-neutral-500 dark:text-neutral-400">
                        Asigna un identificador (ej: Sección A) a un grado específico.
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="overflow-hidden rounded-xl border border-sidebar-border/70 dark:border-sidebar-border">
                        ...
                        <div className="grid gap-6 p-6">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="grade_id" className="text-[10px] font-bold uppercase tracking-wider text-neutral-400">
                                    Grado Académico
                                </Label>
                                <Select
                                    value={data.grade_id?.toString()}
                                    onValueChange={(val) => setData('grade_id', parseInt(val))}
                                >
                                    <SelectTrigger>
